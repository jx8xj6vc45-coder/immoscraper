...
class BetterhomesScraper(BaseScraper):
    """Scraper für betterhomes.ch - Schweizer Immobilienmakler."""

    BASE_URL = 'https://www.betterhomes.ch'
    MAX_PAGES = 3
    ...
    def build_search_url(self, page: int = 1) -> str:
        ...
        # Betterhomes Suche für Kaufobjekte in Zürich
        url = (
            f"{self.BASE_URL}/de/kaufen"
            f"?region=zurich"
    ...
    def search(self) -> List[Listing]:
        ...
        for page in range(1, self.MAX_PAGES + 1):
            logger.info(f"[betterhomes] Lade Seite {page}...")

            page_listings = self._search_page(page)

            if not page_listings:
                logger.info(f"[betterhomes] Seite {page}: keine weiteren Listings")
                break
            ...
            logger.info(f"[betterhomes] Seite {page}: {new_count} neue Listings")
        ...
        logger.info(f"[betterhomes] Total: {len(all_listings)} Listings")
        return all_listings
    ...
    def _search_page(self, page: int) -> List[Listing]:
        ...
        try:
            ...
            logger.debug(f"[betterhomes] Fetching {url}")
            ...
            logger.error(f"[betterhomes] Seite {page} Fehler: {e}")
            return []
        finally:
            if context:
                try:
    ...
    def parse_listings(self, content: str) -> List[Listing]:
        ...
        # Betterhomes verwendet Card-Strukturen
        cards = soup.find_all('a', href=re.compile(r'/de/kaufen/[^?]+'))
        if not cards:
            cards = soup.find_all('article', class_=re.compile(r'property|listing', re.I))
        if not cards:
            cards = soup.find_all('div', class_=re.compile(r'property-card|listing-card|result-item', re.I))

        logger.info(f"[betterhomes] {len(cards)} Listing-Cards gefunden")
        ...
        for card in cards:
            try:
                ...
                logger.debug(f"[betterhomes] Card-Parsing Fehler: {e}")
    ...
    def _parse_card(self, card) -> Listing:
        ...
        if card.name == 'a':
            ...
            link = card.find('a', href=re.compile(r'/de/kaufen/'))
            href = link.get('href', '') if link else ''

        if not href or href == '/de/kaufen' or href.endswith('/de/kaufen/'):
            return None
        ...
        if slug and slug != 'kaufen':
            listing.external_id = f"bh-{slug}"
        else:
            return None
