"""Scraper für Tröndle & Partner Immobilien."""
...
class TroendleScraper(BaseScraper):
    """Scraper für troendle-partner.ch - Zürcher Immobilienmakler."""

    BASE_URL = 'https://www.troendle-partner.ch'
    MAX_PAGES = 3
    ...
    def build_search_url(self, page: int = 1) -> str:
        # Tröndle & Partner Kaufobjekte
        url = f"{self.BASE_URL}/immobilien/kaufen"
        if page > 1:
            url += f"?page={page}"
        return url
    ...
    def search(self) -> List[Listing]:
        ...
        for page in range(1, self.MAX_PAGES + 1):
            logger.info(f"[troendle] Lade Seite {page}...")

            page_listings = self._search_page(page)

            if not page_listings:
                logger.info(f"[troendle] Seite {page}: keine weiteren Listings")
                break
            ...
            logger.info(f"[troendle] Seite {page}: {new_count} neue Listings")
        ...
        logger.info(f"[troendle] Total: {len(all_listings)} Listings")
        return all_listings
    ...
    def _search_page(self, page: int) -> List[Listing]:
        ...
        try:
            ...
            logger.debug(f"[troendle] Fetching {url}")
            ...
            logger.error(f"[troendle] Seite {page} Fehler: {e}")
            return []
        finally:
            if context:
                try:
    ...
    def parse_listings(self, content: str) -> List[Listing]:
        ...
        # Suche nach Immobilien-Links
        cards = soup.find_all('a', href=re.compile(r'/immobilien/[^/]+/\d+'))
        if not cards:
            cards = soup.find_all('a', href=re.compile(r'/objekt/'))
        if not cards:
            cards = soup.find_all('article', class_=re.compile(r'property|listing|immobilie', re.I))
        if not cards:
            cards = soup.find_all('div', class_=re.compile(r'property|listing|result|card', re.I))

        logger.info(f"[troendle] {len(cards)} Listing-Cards gefunden")
        ...
        for card in cards:
            try:
                ...
                logger.debug(f"[troendle] Card-Parsing Fehler: {e}")
    ...
    def _parse_card(self, card) -> Listing:
        ...
        if card.name == 'a':
            ...
            link = card.find('a', href=re.compile(r'/immobilien/|/objekt/'))
        ...
        if href.rstrip('/') in ('/immobilien/kaufen', '/immobilien/mieten', '/immobilien'):
            return None
        ...
        if id_match:
            listing.external_id = f"troendle-{id_match.group(1)}"
        else:
            slug = href.rstrip('/').split('/')[-1]
            listing.external_id = f"troendle-{slug}"
